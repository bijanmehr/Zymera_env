"""Scale the hard connectivity guardrail to the REAL task: 16x16 / 4 agents / comm_r=5
/ 70 steps. Degree-floor baseline there (via curriculum) was 68.9% / 87% connected.
Single-phase, from scratch, guardrail vs soft degree-floor, apples-to-apples.

    python -u examples/run_guardrail_16.py
"""
...
STEPS, N, COMM, CELLS = 70, 4, 5, 256
cc.MAX_STEPS = STEPS
core.T = STEPS
fc.T = STEPS
core.ITERS = 200
ENV = dict(grid_h=16, grid_w=16, n_agents=N, comm_r=COMM, spawn_radius=2)
...
def evalp(env, vdir, guard):
    ...
    cr = int(env.comm_r) if guard else None
    ...
    return cov, float((giant[:, -1] / N).mean()), float((giant == N).mean()), \
        float(jnp.where(~jnp.eye(N, dtype=bool), d[:, -1], 0).max((-1, -2)).mean())


base = core.new_run_dir()
print(f"GUARDRAIL @ 16x16 / 4 agents / comm_r={COMM} / {STEPS} steps · {base}\n", flush=True)
VARIANTS = [
    ("guardrail", dict(), True),
...
for name, ckw, guard in VARIANTS:
    ...
    for sd in SEEDS:
        ...
        b = fc.run(env, M, f"{name}-s{sd}", outdir=base, seed=sd, target_cov=None,
                   patience=999, min_iters=999, mask_collisions=False, guard_conn=guard)
...
print(f"\n  real-task ref (degree-floor curriculum): 68.9% / 87% giant. guardrail target: 100% connected + higher cov.")
